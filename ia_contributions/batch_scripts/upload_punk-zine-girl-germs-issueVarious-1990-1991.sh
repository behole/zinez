#!/bin/bash
# Upload script for Internet Archive
# Zine: Girl Germs
# Generated: 2025-10-30 08:42:28

# Check if internetarchive is configured
if ! ia configure --help &> /dev/null; then
    echo 'Error: internetarchive not configured'
    echo 'Run: ia configure'
    exit 1
fi

# Upload punk-zine-girl-germs-issueVarious-1990-1991
ia upload \
  "punk-zine-girl-germs-issueVarious-1990-1991" \
  --metadata="title:Girl Germs" \
  --metadata="mediatype:texts" \
  --metadata="collection:opensource" \
  --metadata="description:Early Riot Grrrl zine by Bratmobile members

Punk zine from Olympia, Washington, USA
Published: 1990-1991

Featured bands: Bratmobile, Various riot grrrl bands

---
Contributed by the Punk Zines Research Project
Part of documenting underground punk culture and DIY publishing" \
  --metadata="subject:riot grrrl; feminist; punk; Olympia; Bratmobile" \
  --metadata="creator:Allison Wolfe, Molly Neuman" \
  --metadata="date:1990-1991" \
  --metadata="language:eng" \
  --metadata="licenseurl:http://creativecommons.org/licenses/by-nc-sa/4.0/" \
  --metadata="coverage:Olympia, Washington, USA" \
  --metadata="volume:Various"

echo "Upload complete!"
echo "View at: https://archive.org/details/punk-zine-girl-germs-issueVarious-1990-1991"